...
public class ModSlider implements IUIElement {
    ...
    public void render(SpriteBatch sb) {
        ...
        String renderVal = Integer.toString(Math.round(this.value * this.multiplier));
        if (this.sliderGrabbed) {
            FontHelper.renderFontCentered(sb, FontHelper.tipBodyFont, renderVal + this.suffix, this.sliderX + SLIDE_W + 55.0F * Settings.scale, this.y, Settings.GREEN_TEXT_COLOR);
        } else {
            FontHelper.renderFontCentered(sb, FontHelper.tipBodyFont, renderVal + this.suffix, this.sliderX + SLIDE_W + 55.0F * Settings.scale, this.y, Settings.BLUE_TEXT_COLOR);
        }
    ...
    public void update() {
        ...
        int oldVal = Math.round(this.value * this.multiplier);
        this.value = (this.handleX - 11.0F * Settings.scale - this.sliderX) / SLIDE_W;
        if (oldVal != Math.round(this.value * this.multiplier)) {
            this.onChange();
        }
